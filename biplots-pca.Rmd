...
```{r}
# in here do the subtractions to the following variables
# time_series_weekly
# time_series_biweekly
# time_series_monthly
...
#   country_subset_mod <- country_subset
#   # for each column name
#   for (j in colnames(subset(country_subset, select = -c(date, iso_code, reproduction_rate)))) {
#     col.values <- country_subset[[j]] #select one column
#     for (k in seq_along(col.values)) {
#       if (is.na(col.values[[k]])) { # if value = NA
#         if (k == 1) { # if first element -> set = 0
...
# time_series_monthly_only_new_values <- new_t # DANGER
time_series_monthly <- new_t # DANGER
```
...
```{r}
get_lines <- function(cluster_number) {
  countries_names <- row.names(countries_with_clusters[countries_with_clusters$cluster == cluster_number, ])
  months_data <- unique(time_series_monthly$date)
  indexes_1 <- which(time_series_monthly$iso_code %in% countries_names)
  names <- NULL
  x_axis <- NULL
  y_axis <- NULL
  for (month in months_data) {
...
for (i in number_of_clusters) {
  ...
  plot <- ggplot() +
    geom_line(data = df, aes(x = x, y = y, colour = iso_code)) +
    labs(colour = "Country", x = "Deaths, vaccinations and testing", y = "Reproduction and stringency", title = "Evolution of coronavirus within the class")
  print(plot)
}
...
get_lines <- function(cluster_number) {
  countries_names <- row.names(countries_with_clusters[countries_with_clusters$cluster == cluster_number, ])
  months_data <- unique(time_series_monthly_only_new_values$date)
  indexes_1 <- which(time_series_monthly_only_new_values$iso_code %in% countries_names)
  names <- NULL
  x_axis <- NULL
  y_axis <- NULL
  for (month in months_data) {
    indexes_2 <- which(time_series_monthly_only_new_values$date == month)
    indexes <- intersect(indexes_1, indexes_2)
    pca_interesting <- pca
    pca_interesting$x <- pca$x[indexes, ]
    x_axis <- c(x_axis, pca_interesting$x[, 1])
    y_axis <- c(y_axis, pca_interesting$x[, 2])
    names <- c(names, time_series_monthly_only_new_values[indexes, ]$iso_code)
  }
  df <- do.call(data.frame, list(x_axis, y_axis, names))
  colnames(df) <- c("x", "y", "iso_code")
  return(df)
