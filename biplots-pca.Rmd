...
```{r biplotDeltaValuesNoTimeClassesAllData, fig.height=3}
...
for (i in number_of_clusters) {
    df <- get_lines(i, table_no_boosters, pca_new)
...
The following graphs give us another dimension to analyze: the color of points represent timeline, from the earliest dates (darker blue) to the latest (light blue). Here we can clearly see a movement to the upper direction on Y axis with time within each cluster.
...
```{r PCA1evolutionOverTimeDeltaValues}
number_of_clusters <- unique(countries_with_clusters$cluster)
for (i in number_of_clusters) {
  df <- get_lines(i, table_no_boosters, pca_new)
  plot <- ggplot() +
...
```{r biplotDeltaValuesWithoutTimeClasses}
table_no_boosters <- subset(time_series_monthly_only_new_values, select = -c(new_boosters))
...
```{r biplotDeltaValuesWithTimeClasses}
#  Display plot for new value for each countries cluster
number_of_clusters <- unique(countries_with_clusters$cluster)
for (i in number_of_clusters) {
  df <- get_lines(i, table_no_boosters, pca_new_no_boosters)
...
```{r biplotDeltaValuesWithTimeIndividual}
