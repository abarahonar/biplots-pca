...
get_lines <- function(cluster_number, table, pca) {
  ...
  df <- do.call(data.frame, list(x_axis, y_axis, names, as.Date(c(months), "%Y-%m-%d"), clusters))
  colnames(df) <- c("x", "y", "iso_code", "date", "cluster")
  return(df)
}
```
...
```{r biplotDeltaValuesWithBoostersNoTimeClassesAllData}
...
### Biplot of data with boosters, grouped by countries

```{r biplotDeltaValuesWithBoostersNoTimeClasses}
#  Display plot for accumulative value for each countries cluster
number_of_clusters <- unique(countries_with_clusters$cluster)
for (i in number_of_clusters) {
  df <- get_lines(i, time_series_monthly_only_new_values, pca_new)
