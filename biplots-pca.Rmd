...
The variables present in the data set can be used to represent two types of information: the evolution of the pandemic as a time series and metrics regarding the situation of the countries as a whole. The former correspond to dynamic data while the latter is static data.
...
After processing those variables, the corresponding data set regarding the countries contains `r count(countries)` entries. However, not every country started reporting at the same time, so in some situations there are noticeable differences in the starting reporting dates. A sample of the starting reporting dates can be seen in the Table \@ref(tab:reportingdates).
...
kable(start.reporting.dates.table, caption="First and last 5 countries and their starting reporting dates. Alphabeticaly sorted")
```

```{r echo=FALSE}
good.fast.countries <- start.reporting.dates[start.reporting.dates$date < "2020-05-01",]
countries <- countries[countries$iso_code %in% good.fast.countries$iso_code,]

```

The date 2020-05-01 was selected as the threshold with which countries are removed from the data set. If they started reporting at a latter date, they are not considered. The remaining countries consists of `r count(countries)` entries.

```{r echo=FALSE}
countries <- na.omit(countries)
```
The final trimming consists in eliminate entries with one or more NA values. The final data set consists of `r count(countries)` entries. To see the behaviour of the data set, refer to the Figure \@ref(fig:boxplotsCountries).
<!-- This redaction needs some serious checking, please help me Natasha -->

```{r boxplotsCountries, fig.cap="Currently working on a title",echo=FALSE}
library(ggplot2)
library(reshape2)

# The scales are way too fucking big, we should normalize
...
Several columns and rows are not necessary, so they are deleted. The list of deleted rows and variables and the reason is:

* 16161 rows corresponding to physical aggregations of data
* Continent and location columns due to redundancy <!-- It may be necessary -->
* Columns related to mortality rates due to it having 83% missing values
* Hand washing facilities column due to it having 45% missing values and not being relevant to the study
* Columns related to ICU and hospitalization rates due to it having at least 75% missing values
* Smoothed columns due to it being time considered and the way the value is obtained is not relevant to the study
* Columns Relative variables due to it not being a comparable measure
* Columns related to age due to it being unnecessary to this study
* Positive rate column due to it having 42% missing values
<!-- Put the percetange of missing values in code -->
