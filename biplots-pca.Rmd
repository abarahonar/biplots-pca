...
```{r pcaCreation, fig.cap="Variance of each components in the total data", fig.height=3}
...
```{r pcaNewCreation, fig.cap="Variance of each components in the delta data", fig.height=3}
...
```{r pcaMainBiplot, fig.cap="Biplot projection over the first two component in the delta data"}
...
For the remainder of this document, the data set that is used correspond to the delta data, which is going to be referred from this point onwards as data.
...
```{r clusterHumanMedian, fig.cap="Distribution of the classes given the human development index and median age"}
ggplot(countries_with_clusters, aes(x=human_development_index, y=median_age, color = factor(cluster))) + geom_point()
```

```{r clusterDensityHospitalBeds, fig.cap="Distribution of the classes given population density and hospital beds per thousand"}
...
On the next stage of the project, we aimed to have a closer look at the behavior of the data within clusters. For that, we first plotted all the data for each cluster on the same plane. Taking into consideration the visualisation results of the PCA analysis (Figure 10), the axes can be interpreted as following: the values that are the most significat for defining the value of the first PCA component are new deaths and new cases, so we interprit the X axis as the composition of them. The Y axis is most influenced by the new_tests and new_vaccinations parameters, therefore Y represents a composition of these two. 
As it can be read from the Figure 10, the values of both PCA components and two above named data compositions are inversely proportional, which means that with the increment of X value the number of new deaths and new cases decreases, and the increment of Y means recesssion of new_tests and new_vaccinations.
...
```{r biplotDeltaValuesWithBoostersNoTimeClasses, fig.height = 3, fig.width = 4}
...
```
Analyzing  the data within each single cluster we did not really notice any specific behavior for each country. Therefore we decided to take the total data for each cluster and compare it with one another. In the (Figure x) we can notice that for each class the data is indeed accumulated area of the graph which means there can be some kind of correlation to explore.
More than that, though we could not extract a specific behavior for a country, we can still notice a common shape formed by the data points for each cluster: each graph tends to display a line which goes from the top left to the bottom right. That gave us a hint to have a look at how the data behaves on the timeline.

```{r biplotDeltaValuesWithBoostersNoTimeClasses, fig.height = 3, fig.width = 4}
colors = c("yellow", "red", "blue", "green", "black", "orange", "purple", "cyan")
common_plot <- ggplot() +
    labs(colour = "Country", x = "New deaths & cases", y = "vaccination & testing", title = "Evolution of coronavirus within the class")
for (i in number_of_clusters) {
   common_plot <- common_plot + geom_point(data=data_sets[[i]], aes(x = x, y = y), colour=colors[i]) 
}
  print(common_plot)

```
The following graphs give us another dimension to analyse: the color of points represent timeline, from the earliest dates (darker blue) to the latest (light blue). Here we can clearly see a movement to the upper direction on Y axis with time within each cluster.
```{r biplotDeltaValuesWithBoostersWithTimeClasses}
...
```{r biplotDeltaValuesWithBoostersWithTimeIndividual}
...
```{r PCA2evolutionOverTimeDeltaValuesWithBoosters}
#  Display plot for new value for each countries cluster
number_of_clusters <- unique(countries_with_clusters$cluster)
for (i in number_of_clusters) {
  df <- get_lines(i, time_series_monthly_only_new_values, pca_new)
...
```{r PCA1evolutionOverTimeDeltaValuesWithBoosters}
number_of_clusters <- unique(countries_with_clusters$cluster)
for (i in number_of_clusters) {
  df <- get_lines(i, time_series_monthly_only_new_values, pca_new)
  plot <- ggplot() +
...
```{r biplotDeltaValuesNoBoostersWithoutTimeClasses}
...
```{r biplotDeltaValuesNoBoostersWithTimeClasses}
#  Display plot for new value for each countries cluster
number_of_clusters <- unique(countries_with_clusters$cluster)
for (i in number_of_clusters) {
  df <- get_lines(i, table_no_boosters, pca_new_no_boosters)
...
```{r biplotDeltaValuesNoBoostersWithTimeIndividual}
#  Display trajectory for each eu country + Chile
...
```{r somePlotsOfCountriesVariables}
