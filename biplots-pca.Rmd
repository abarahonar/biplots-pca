...
# TODO: color
makeplot <- function(date, my_colors) {
  interesting_countires <- c(
    "RUS",
    "CHL",
    "CHN",
  ...
  indexes_1 <- which(time_series_monthly$iso_code %in% interesting_countires)
  indexes_2 <- which(time_series_monthly_pca$date == date)
  indexes <- intersect(indexes_1, indexes_2)
  names <- time_series_monthly[indexes, ]$iso_code
  pca_interesting <- pca
  pca_interesting$x <- pca$x[indexes, ]

  l_x <- pca$rotation[, 1] * 4
  l_y <- pca$rotation[, 2] * 4
...
```{r video}
library(av, warn.conflicts = FALSE)
png("biplots-pca_files/images/input%03d.png", width = 1280, height = 720, res = 108)
my_cols <- palette(rainbow(10)) 
makeplot(1, my_cols)
makeplot(1, my_cols)
for(i in 1:17) {
  makeplot(i, my_cols)
...
makeplot(17, my_cols)
dev.off()
png_files <- sprintf("biplots-pca_files/images/input%03d.png", 1:38)
av::av_encode_video(png_files, "biplots-pca_files/videos/output.mp4", framerate=1.5)
utils::browseURL("biplots-pca_files/videos/output.mp4")
```
